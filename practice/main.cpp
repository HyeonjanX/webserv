...
/*   Updated: 2023/08/29 23:02:00 by gychoi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	change_events(change_list, sockfd, EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, NULL);

	int				new_events;
	struct kevent*	curr_events;
	while (true)
	{
		...
		for (int i = 0; i < new_events; ++i)
		{
			...
			else if (curr_event->filter == EVFILT_READ)
			{
				if (curr_event->ident == sockfd)
				{
					...
					change_events(change_list, connfd, EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, NULL);
					change_events(change_list, connfd, EVFILT_WRITE, EV_ADD | EV_ENABLE, 0, 0, NULL);
					clients[connfd] = "";
				...
				else if (clients.find(curr_event->ident) != clients.end())
				{
					...
					while (true)
					{
						...
						{
							close(sockfd);
							disconnect_client(curr_event->ident, clients); // 한 번에 모든 클라이언트를 지우러면?
							error("Fatal: recv");
						}
						else if (ret == 0)
						{
						...
						buffer[ret] = '\0';
						clients[curr_event->ident].append(buffer);
						if (isAllReceived(clients[curr_event->ident]))
							break;
					}
				}
			}
			else if (curr_event->filter == EVFILT_WRITE)
			{
				...
				if (it != clients.end())
				{
					...
					{
						if (clients[curr_event->ident].find("HTTP") == std::string::npos)
							error("Malformed http request");
						std::size_t	startPos = clients[curr_event->ident].find("GET ");
						std::size_t	endPos;
						std::string	filename;
						if (startPos != std::string::npos)
						{
							...
							startPos += 4;
							endPos = clients[curr_event->ident].find(' ', startPos);
							filename = clients[curr_event->ident].substr(startPos, endPos - startPos);
							if (!filename.empty() && filename[0] == '/')
								filename = filename.substr(1);
							if (filename.empty())
							{
								ret = send(curr_event->ident, SERVER_MSG.c_str(), SERVER_MSG.size(), 0);
								if (ret == -1)
								{
									close(sockfd);
									close(connfd);
							...
							else
							{
								...
								{
									close(sockfd);
									disconnect_client(curr_event->ident, clients); // 한 번에 모두 해제하려면?
								...
								while ((entry = readdir(dir)) != NULL)
								{
									if (entry->d_type == DT_REG && std::string(entry->d_name) == filename)
									{
										...
										if (file.is_open())
										{
											...
											ret = send(curr_event->ident, response.c_str(), response.size(), 0);
											if (ret == -1)
											{
												close(sockfd);
												closedir(dir);
										...
										{
											ret = send(curr_event->ident, ERROR_MSG.c_str(), ERROR_MSG.size(), 0);
											if (ret == -1)
											{
												close(sockfd);
												closedir(dir);
						...
						else if (clients[curr_event->ident].find("POST ") != std::string::npos)
						{
